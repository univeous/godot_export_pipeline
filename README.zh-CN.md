...
对于大多数游戏，裁剪后的导出模板可以小于 30 MB；这里只指导出模板本身，不包括 PCK。

PCK 修剪的效果见仁见智。如果您提前规划好了，项目里的所有资源都用得上，那它几乎没什么用（不过仍然可以帮您剔除用不到的开发期插件和其他不该进入成品的文件）；但如果您喜欢在项目里放各种东西来实验，PCK 修剪会很有用。
